...
TEXTONLY_HEADER = "请根据这段文本转录判断该说话人是抑郁还是非抑郁"
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Convert multiprompt JSONL files into the text-only prompt template."
    )
    ...
    parser.add_argument(
        "--generate-cmdc-5fold",
        action="store_true",
        help="Generate text-only files for all CMDC fold train/test multiprompt files.",
    )
...
def convert_prompt(prompt: str) -> str:
    prompt = prompt.replace(AUDIO_TOKENS, "", 1)
    if FULL_HEADER in prompt:
        prompt = prompt.replace(FULL_HEADER, TEXTONLY_HEADER, 1)

    transcript_index = prompt.find(TRANSCRIPT_MARKER)
    if transcript_index == -1:
        raise ValueError("Prompt is missing the transcript marker.")

    transcript = prompt[transcript_index:].strip()
    return f"{TEXTONLY_HEADER}\n{transcript}"
